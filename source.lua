-- Octernel UI Library - Mobile & Desktop Compatible
-- Version 1.0
...
local library = {
    ...
    tabSize = 0, 
    theme = {}, 
    foldername = "octernel_configs", 
    fileext = ".txt",
    isMobile = (inputService.TouchEnabled and not inputService.KeyboardEnabled and not inputService.MouseEnabled)
}
...
function library:AddTab(title, pos)
    ...
    function tab:AddColumn()
        ...
        function column:Init()
            ...
            -- Adjust column width and position for mobile
            local columnWidth = library.isMobile and 184 or 233
            local columnSpacing = library.isMobile and 190 or 239
            local scrollThickness = library.isMobile and 8 or 4

            self.main = library:Create("ScrollingFrame", {
                ZIndex = 2,
                Position = UDim2.new(0, 6 + (self.position * columnSpacing), 0, 2),
                Size = UDim2.new(0, columnWidth, 1, -4),
                BackgroundTransparency = 1,
                BorderSizePixel = 0,
...
function library:Close()
    ...
    if self.main then
        ...
        if self.toggleButton then
            self.toggleButton.Text = self.open and "✕" or "☰"
        end
    end
end
...
function library:Init()
    ...
    -- Adjust UI size based on device
    local uiWidth = self.isMobile and 380 or 500
    local uiHeight = self.isMobile and 500 or 600
    local uiPosX = self.isMobile and 10 or 100
    local uiPosY = self.isMobile and 70 or 46

    self.main = self:Create("ImageButton", {
        AutoButtonColor = false,
        Position = UDim2.new(0, uiPosX, 0, uiPosY),
        Size = UDim2.new(0, uiWidth, 0, uiHeight),
        BackgroundColor3 = Color3.fromRGB(20, 20, 20),
        BorderColor3 = Color3.new(),
        ScaleType = Enum.ScaleType.Tile,
        Modal = self.isMobile, -- Modal on mobile to prevent accidental clicks
        Visible = false,
        Parent = self.base
    })
    ...
    if self.isMobile then
        ...
        table.insert(library.theme, self.toggleButton)
    ...
    -- Handle both mouse and touch for dragging main window
    ...
    self.top.InputChanged:connect(function(input)
        if dragging and (input.UserInputType.Name == "MouseMovement" or input.UserInputType == Enum.UserInputType.Touch) then
            dragInput = input
        end
    end)
